...
int mode = 0;  //Seleccion distancia = 0, acercamiento = 1, parada = 2, emergencia = 3
    ...
    if (Serial.available() >= 9) {
        ...
        if (0 == strcmp("SPD: REQ\n",request)) {
            ...
            Serial.write(answer);
             ...
             // send the answer for run emergency mode request
            accelerator = false;  //Desactivamos acelerador
            brake = true;  //Activamos freno
            focus = true;  //Activamos focos
            mode = 3;  //Modo emergencia
...
void actualSpeedFunction() {
    ...
    if (mode == 0 || mode == 1 || mode == 3) {  //Si no estamos en modo parada, velocidad normal
...
// --------------------------------------
// Function: emergencyModeScheduler
// --------------------------------------
void emergencyModeScheduler() {
    int timeStart, timeEnd, time;  //Variables para controlar el tiempo
    int wait;  //Variable para saber cuanto esperar
    int secCycle = 0;  //Ciclo secundario en el que se está, 0 o 1
    ...
    while(mode == 3) {  //Mientras modo emergencia
